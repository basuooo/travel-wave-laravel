...
@section('title', $page->localized('meta_title') ?: ($page->localized('title') ?: 'فيزا فرنسا 2'))
...
<div class="tw-france2-page pb-5" dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
    @include('partials.frontend.form-zone', ['assignments' => $managedForms['top'] ?? [], 'position' => 'top', 'sourcePage' => 'france-2'])
    ...
    @foreach($orderedSections as $sKey => $sMeta)
        ...
        @switch($sKey)
            ...
            @case('hero')
                ...
                @include('partials.frontend.form-zone', ['assignments' => $managedForms['below_hero'] ?? [], 'position' => 'below_hero', 'sourcePage' => 'france-2'])
                @break
                ...
                <section class="container py-4">
                    <div class="tw-card p-4 p-md-5 rounded-4 shadow-sm border border-secondary border-opacity-10 text-center mx-auto" style="max-width: 900px;">
                        <h2 class="tw-section-title h3 fw-bold mb-3">
                            {{ $isRtl ? ($intro['title_ar'] ?? 'فيزا فرنسا') : ($intro['title_en'] ?? 'France Visa') }}
                        </h2>
                        <p class="fs-5 text-muted mb-0 leading-relaxed">
                            {{ $isRtl ? ($intro['description_ar'] ?? '') : ($intro['description_en'] ?? '') }}
                        </p>
                    </div>
                </section>
                @endif
                ...
                <section class="container py-4">
                    <div class="text-center mb-4">
                        <h2 class="tw-section-title h3 fw-bold mb-2">
                            {{ $isRtl ? ($req['title_ar'] ?? 'إيه أهم المتطلبات؟') : ($req['title_en'] ?? 'Key Requirements') }}
                        </h2>
            ...
            @case('steps')
                ...
                <section class="container py-4">
                    <div class="text-center mb-4">
                        <h2 class="tw-section-title h3 fw-bold mb-2">
                            {{ $isRtl ? ($steps['title_ar'] ?? 'إجراءات التقديم ببساطة') : ($steps['title_en'] ?? 'Application Steps') }}
                        </h2>
                        ...
                        @foreach(collect($steps['items'] ?? [])->filter(fn($i) => !isset($i['is_active']) || $i['is_active'])->sortBy('sort_order') as $item)
                            <div class="col-sm-6 col-lg-3">
                                <div class="tw-card p-4 h-100 rounded-4 text-center border border-secondary border-opacity-10 shadow-sm">
                                    <div class="badge bg-primary text-white rounded-circle fs-5 fw-bold mb-3 mx-auto d-flex align-items-center justify-content-center" style="width: 50px; height: 50px;">
                                        {{ $item['step_number'] ?? sprintf('%02d', $loop->iteration) }}
                                    </div>
                                    <h3 class="h5 fw-bold mb-2">{{ $isRtl ? ($item['title_ar'] ?? '') : ($item['title_en'] ?? '') }}</h3>
                                    <p class="text-muted small mb-0">{{ $isRtl ? ($item['text_ar'] ?? '') : ($item['text_en'] ?? '') }}</p>
                                </div>
                            </div>
                        @endforeach
                        ...
                        @foreach(collect($serv['items'] ?? [])->filter(fn($i) => !isset($i['is_active']) || $i['is_active'])->sortBy('sort_order') as $item)
                            <div class="col-md-6 col-lg-4">
                                <div class="tw-card p-4 h-100 rounded-4 border border-secondary border-opacity-10 shadow-sm d-flex align-items-start gap-3">
                                    <span class="fs-2 text-primary">{{ $item['icon'] ?? '✔️' }}</span>
                                    <div>
                ...
                <section class="container py-4">
                    <div class="tw-card p-4 p-md-5 rounded-4 shadow-sm border border-primary border-opacity-25 bg-primary bg-opacity-10 text-center mx-auto" style="max-width: 900px;">
                        ...
                        <a href="{{ $suit['button_url'] ?? '/contact#lead-form' }}" class="btn btn-primary btn-lg tw-btn-primary px-5 py-3 rounded-pill fw-bold">
                            {{ $isRtl ? ($suit['button_text_ar'] ?? 'تواصل معنا') : ($suit['button_text_en'] ?? 'Contact Us') }} 💬
                        </a>
                    </div>
                </section>
                @endif
                ...
                <section class="container py-4">
                    <div class="tw-card p-4 p-md-5 rounded-4 shadow-sm border border-secondary border-opacity-10 mx-auto" style="max-width: 900px;">
                        <h2 class="tw-section-title h3 fw-bold text-center mb-4">
                            {{ $isRtl ? ($faq['title_ar'] ?? 'أسئلة شائعة عن فيزا فرنسا') : ($faq['title_en'] ?? 'France Visa FAQs') }}
                        </h2>
                        <div class="accordion" id="france2FaqAccordion">
                            @foreach(collect($faq['items'] ?? [])->filter(fn($i) => !isset($i['is_active']) || $i['is_active'])->sortBy('sort_order') as $item)
                                <div class="accordion-item border rounded-3 mb-2 overflow-hidden">
                                    <h3 class="accordion-header">
                                        <button class="accordion-button fw-bold {{ $loop->first ? '' : 'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#f2-faq-{{ $loop->iteration }}">
                                            {{ $isRtl ? ($item['question_ar'] ?? '') : ($item['question_en'] ?? '') }}
                                        </button>
                                    </h3>
                                    <div id="f2-faq-{{ $loop->iteration }}" class="accordion-collapse collapse {{ $loop->first ? 'show' : '' }}" data-bs-parent="#france2FaqAccordion">
    ...
    @include('partials.frontend.form-zone', ['assignments' => $managedForms['middle'] ?? [], 'position' => 'middle', 'sourcePage' => 'france-2'])
    @include('partials.frontend.form-zone', ['assignments' => $managedForms['bottom'] ?? [], 'position' => 'bottom', 'sourcePage' => 'france-2'])
</div>
@endsection
