...
@php($sidebarSections = [
    ...
    [
        ...
        'items' => array_values(array_filter([
            ...
            ($modDestinationsEnabled && ($adminUser?->hasPermission('visa_database.view') || $adminUser?->hasPermission('destinations.manage') || $adminUser?->is_admin)) ? ['label' => '🌐 قاعدة بيانات التأشيرات (Visa Database)', 'route' => 'admin.visa-database.index', 'match' => 'admin.visa-database.*'] : null,
            ($adminUser?->hasPermission('embassy_appointments.view') || $adminUser?->hasPermission('destinations.manage') || $adminUser?->is_admin) ? ['label' => '🏛️ مواعيد السفارات', 'route' => 'admin.embassy-appointments.index', 'match' => 'admin.embassy-appointments.*'] : null,
    ...
    [
        ...
        'items' => $modMarketingEnabled ? array_values(array_filter([
            ...
            $adminUser?->hasPermission('settings.manage') ? ['label' => '⚡ ربط Zapier', 'route' => 'admin.zapier.index', 'match' => 'admin.zapier.*'] : null,
...
(function() {
    ...
    function updateIconPreview(previewEl, rawVal) {
        if (!previewEl) return;
        const val = (rawVal || '').trim();
        if (!val) {
            previewEl.innerHTML = '<svg viewBox="0 0 24 24" style="width:1.25em; height:1.25em; fill:none; stroke:currentColor; stroke-width:1.8;"><path d="' + presetSvgs.sparkles + '"></path></svg>';
            return;
        }
        if (val.startsWith('<') || val.includes('<svg') || val.includes('<i')) {
            previewEl.innerHTML = val;
            return;
        }
        if (/^[a-z0-9]+(?:-[a-z0-9]+)*:[a-z0-9]+(?:-[a-z0-9]+)*$/i.test(val)) {
            previewEl.innerHTML = '<iconify-icon icon="' + val + '" width="22" height="22"></iconify-icon>';
            return;
        }
        if (val.includes('fa-') || val.includes('bi-') || val.includes('ri-')) {
            previewEl.innerHTML = '<i class="' + val + '"></i>';
            return;
        }
        const key = val.toLowerCase();
        if (presetSvgs[key]) {
            previewEl.innerHTML = '<svg viewBox="0 0 24 24" style="width:1.25em; height:1.25em; fill:none; stroke:currentColor; stroke-width:1.8; stroke-linecap:round; stroke-linejoin:round;"><path d="' + presetSvgs[key] + '"></path></svg>';
            return;
        }
        previewEl.innerHTML = '<svg viewBox="0 0 24 24" style="width:1.25em; height:1.25em; fill:none; stroke:currentColor; stroke-width:1.8;"><path d="' + presetSvgs.sparkles + '"></path></svg>';
    }
    ...
    document.addEventListener('change', function(e) {
        if (e.target.matches('[data-icon-select]')) {
            const group = e.target.closest('.tw-icon-picker-group');
            if (!group) return;
            const input = group.querySelector('[data-icon-input]');
            const preview = group.querySelector('[data-icon-preview]');
            const val = e.target.value;
            if (val !== 'custom') {
                input.value = val;
                updateIconPreview(preview, val);
            } else {
                input.focus();
                updateIconPreview(preview, input.value);
            }
        }
    });
    ...
    document.addEventListener('input', function(e) {
        if (e.target.matches('[data-icon-input]')) {
            const group = e.target.closest('.tw-icon-picker-group');
            if (!group) return;
            const select = group.querySelector('[data-icon-select]');
            const preview = group.querySelector('[data-icon-preview]');
            const val = e.target.value.trim();
            if (select) {
                let matched = false;
                for (let i = 0; i < select.options.length; i++) {
                    if (select.options[i].value && select.options[i].value === val.toLowerCase()) {
