...
@section('page_description', 'Upload the banner image and manage optional bilingual text, CTA, order, and active state.')
...
<form method="post" enctype="multipart/form-data" action="{{ $item->exists ? route('admin.hero-slides.update', $item) : route('admin.hero-slides.store') }}">
    ...
    <div class="card admin-card p-4">
        <div class="row g-3">
            <div class="col-md-6">
                <label class="form-label">Headline EN <span class="text-muted small">(Optional)</span></label>
                <input class="form-control" name="headline_en" value="{{ old('headline_en', $item->headline_en) }}">
            </div>
            <div class="col-md-6">
                <label class="form-label">Headline AR <span class="text-muted small">(Optional)</span></label>
                <input class="form-control text-end" dir="rtl" name="headline_ar" value="{{ old('headline_ar', $item->headline_ar) }}">
            </div>
            <div class="col-md-6">
                <label class="form-label">Subtitle EN <span class="text-muted small">(Optional)</span></label>
                <textarea class="form-control" name="subtitle_en" rows="3">{{ old('subtitle_en', $item->subtitle_en) }}</textarea>
            </div>
            <div class="col-md-6">
                <label class="form-label">Subtitle AR <span class="text-muted small">(Optional)</span></label>
                <textarea class="form-control text-end" dir="rtl" name="subtitle_ar" rows="3">{{ old('subtitle_ar', $item->subtitle_ar) }}</textarea>
            </div>
            <div class="col-md-4">
                <label class="form-label">CTA Text EN <span class="text-muted small">(Optional)</span></label>
                <input class="form-control" name="cta_text_en" value="{{ old('cta_text_en', $item->cta_text_en) }}">
            </div>
            <div class="col-md-4">
                <label class="form-label">CTA Text AR <span class="text-muted small">(Optional)</span></label>
                <input class="form-control text-end" dir="rtl" name="cta_text_ar" value="{{ old('cta_text_ar', $item->cta_text_ar) }}">
            </div>
            <div class="col-md-4">
                <label class="form-label">CTA Link <span class="text-muted small">(Optional)</span></label>
