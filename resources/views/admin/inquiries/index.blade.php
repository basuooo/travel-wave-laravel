...
@section('page_description', 'Track leads by form, type, status, date, source page, and display position.')
@section('content')
...
<form class="card admin-card p-4 mb-4">
    <div class="row g-3 align-items-end">
        ...
        <div class="col-md-3"><label class="form-label">Form</label><select class="form-select" name="form"><option value="">All</option>@foreach($forms as $form)<option value="{{ $form->id }}" @selected((string) request('form') === (string) $form->id)>{{ $form->name }}</option>@endforeach</select></div>
        ...
        <div class="col-md-3 d-flex gap-2"><button class="btn btn-primary">Filter</button><a href="{{ route('admin.forms.submissions') }}" class="btn btn-outline-secondary">Reset</a></div>
    </div>
</form>
<div class="card admin-card p-4">
    <div class="table-responsive">
        <table class="table align-middle">
            <thead><tr><th>Name</th><th>Form</th><th>Source</th><th>Status</th><th>Date</th><th></th></tr></thead>
            <tbody>
                @foreach($items as $item)
                    <tr>
                        <td><div class="fw-semibold">{{ $item->full_name }}</div><div class="text-muted small">{{ $item->phone }}</div></td>
                        <td>
                            <div class="fw-semibold">{{ $item->form_name ?: ($item->form?->name ?: ucfirst($item->type)) }}</div>
                            <div class="text-muted small">{{ $item->form_category ?: $item->type }}</div>
                        </td>
                        <td>
                            <div>{{ $item->source_page ?: '-' }}</div>
                            <div class="text-muted small">{{ $item->display_position ?: '-' }}</div>
                        </td>
