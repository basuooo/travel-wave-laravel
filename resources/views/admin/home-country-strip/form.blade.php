...
<form method="post" enctype="multipart/form-data" action="{{ $item->exists ? route('admin.home-country-strip.update', $item) : route('admin.home-country-strip.store') }}">
    ...
    <div class="card admin-card p-4">
        <div class="row g-3">
            ...
            <div class="col-md-6">
                <label class="form-label fw-bold">Flag Image (علم الدولة)</label>
                @include('admin.partials.world-flag-picker', [
                    'currentValue' => $item->flag_image_path,
                    ...
                    <div class="row g-3">
                        ...
                        @if($item->flag_image_path)
                            <div class="col-md-6">
                                ...
                                <img src="{{ $item->displayFlagUrl() }}" alt="{{ $item->displayName('en') }} flag" class="img-fluid rounded-4 border" style="max-height: 140px; object-fit: contain; background:#fff;">
                            </div>
                        @endif
                    </div>
                </div>
