...
class LeadFormController extends Controller
{
    ...
    protected function validatedData(Request $request, ?int $formId = null): array
    {
        $validated = $request->validate([
            ...
            'fields.*.depends_on_field' => ['nullable', 'string', 'max:255'],
            'fields.*.depends_on_value' => ['nullable', 'string', 'max:255'],
        ...
        return [
            'form' => [
                ...
                'settings' => [
                    ...
                    'thank_you' => [
                        'action' => $request->input('settings.thank_you.action', 'thank_you_page'),
                        'title_ar' => $request->input('settings.thank_you.title_ar'),
                        'title_en' => $request->input('settings.thank_you.title_en'),
                        'message_ar' => $request->input('settings.thank_you.message_ar'),
                        'message_en' => $request->input('settings.thank_you.message_en'),
                        'bg_color' => $request->input('settings.thank_you.bg_color', '#ffffff'),
                        'text_color' => $request->input('settings.thank_you.text_color', '#212529'),
                        'redirect_url' => $request->input('settings.thank_you.redirect_url'),
                        'custom_html' => $request->input('settings.thank_you.custom_html'),
                        'custom_css' => $request->input('settings.thank_you.custom_css'),
                    ],
                ],
            ],
    ...
    protected function mapFields(array $fields): array
    {
        return collect($fields)->map(function (array $field, int $index) {
            ...
            return [
                ...
                'options' => $options,
                'depends_on_field' => trim((string) ($field['depends_on_field'] ?? '')),
                'depends_on_value' => trim((string) ($field['depends_on_value'] ?? '')),
